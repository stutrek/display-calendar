...
import { type EnrichedEvent, formatTimeRange, useCalendar } from './CalendarContext';
    ...
    <button class="event-item" onClick={onClick} type="button">
      <div class="event-colors">
        {event.colors.map((color, i) => (
          <span key={i} class="event-color-bar" style={{ backgroundColor: color }} />
        ))}
      </div>
  ...
  return (
    <div class="event-list">
      ...
      {timedEvents.map((event, i) => (
        <EventItem key={`timed-${i}`} event={event} onClick={() => setSelectedEvent(event)} />
      ))}

      {/* Divider between timed and all-day */}
      {hasTimedEvents && hasAllDayEvents && <div class="event-divider" />}

      {/* All-day events */}
      {allDayEvents.map((event, i) => (
        <EventItem key={`allday-${i}`} event={event} onClick={() => setSelectedEvent(event)} />
      ))}
