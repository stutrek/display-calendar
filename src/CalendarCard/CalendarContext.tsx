...
export function CalendarProvider({
  ...
  // Day-by-day navigation, used when the month grid is hidden. Crossing a
  // month boundary also moves currentMonth so the events fetch covers the
  // new day's range.
  const nextDay = useCallbackStable(() => {
    setSelectedDay((prev) => {
  ...
  const prevDay = useCallbackStable(() => {
    setSelectedDay((prev) => {
      const next = new Date(prev.getFullYear(), prev.getMonth(), prev.getDate() - 1);
      if (next.getMonth() !== prev.getMonth() || next.getFullYear() !== prev.getFullYear()) {
        setCurrentMonth(new Date(next.getFullYear(), next.getMonth(), 1));
...
export type { CalendarEventWithSource, WeatherForecast } from 'preact-homeassistant';
